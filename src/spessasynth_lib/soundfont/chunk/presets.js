...
export class Preset {
  ...
  getPresetZones(amount, zones) {
    ...
    for (
      let i = this.presetZoneStartIndex;
      i < this.presetZonesAmount + this.presetZoneStartIndex;
      i++
    ) {
  ...
  getSamplesAndGenerators(midiNote, velocity) {
    ...
    function identicalMod(mod1, mod2) {
      return (
        mod1.modulatorSource === mod2.modulatorSource &&
     ...
     * @type {Generator[]}
     */
    let globalPresetGenerators = this.presetZones[0].isGlobal
      ? [...this.presetZones[0].generators]
      : []

    let globalPresetModulators = this.presetZones[0].isGlobal
      ? [...this.presetZones[0].modulators]
      : []

    // find the preset zones in range
    let presetZonesInRange = this.presetZones.filter(
      (currentZone) =>
    ...
    presetZonesInRange.forEach((zone) => {
       ...
       * @type {Generator[]}
       */
      let globalInstrumentGenerators = zone.instrument.instrumentZones[0].isGlobal
        ? [...zone.instrument.instrumentZones[0].generators]
        : []
      let globalInstrumentModulators = zone.instrument.instrumentZones[0].isGlobal
        ? [...zone.instrument.instrumentZones[0].modulators]
        : []

      let instrumentZonesInRange = zone.instrument.instrumentZones.filter(
        (currentZone) =>
          isInRange(currentZone.keyRange.min, currentZone.keyRange.max, midiNote) &&
          isInRange(currentZone.velRange.min, currentZone.velRange.max, velocity) &&
          !currentZone.isGlobal
...
export function readPresets(presetChunk, presetZones) {
  ...
  while (presetChunk.chunkData.length > presetChunk.chunkData.currentIndex) {
    ...
    if (presets.length > 0) {
      let presetZonesAmount =
        preset.presetZoneStartIndex - presets[presets.length - 1].presetZoneStartIndex
