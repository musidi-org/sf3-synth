...
export class InstrumentZone {
  ...
  getGenerators(generators) {
    for (
      let i = this.generatorZoneStartIndex;
      i < this.generatorZoneStartIndex + this.generatorZoneSize;
      i++
    ) {
  ...
  getModulators(modulators) {
    for (
      let i = this.modulatorZoneStartIndex;
      i < this.modulatorZoneStartIndex + this.modulatorZoneSize;
      i++
    ) {
 ...
 */
export function readInstrumentZones(
  zonesChunk,
  ...
  while (zonesChunk.chunkData.length > zonesChunk.chunkData.currentIndex) {
    ...
    if (zones.length > 0) {
      let modulatorZoneSize =
        zone.modulatorZoneStartIndex - zones[zones.length - 1].modulatorZoneStartIndex
      let generatorZoneSize =
        zone.generatorZoneStartIndex - zones[zones.length - 1].generatorZoneStartIndex
