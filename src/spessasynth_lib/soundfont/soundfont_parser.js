...
export class SoundFont2 {
  ...
  constructor(dataArray) {
    ...
    while (infoChunk.chunkData.length > infoChunk.chunkData.currentIndex) {
      ...
      console.log(`%c"${chunk.header}": %c"${text}"`, consoleColors.info, consoleColors.recognized)
    ...
    let presetZones = readPresetZones(
      presetZonesChunk,
      presetGenerators,
      presetModulators,
      instruments
  ...
  getPreset(bankNr, presetNr) {
    ...
    if (!preset) {
      ...
      if (preset) {
        console.info(
          `%cPreset at bank ${bankNr} not found. Replaced with ${preset.presetName}`,
          consoleColors.warn
        )
      }
    }
    ...
    while (soundfonts.length) {
      ...
      newPresets.forEach((newPreset) => {
        if (
          presets.find(
            (existingPreset) =>
              existingPreset.bank === newPreset.bank && existingPreset.program === newPreset.program
          ) === undefined
        ) {
