...
import {
  ...
  midiControllers,
  MidiMessage
} from '../midi_parser/midi_message.js'
...
export class Sequencer {
  ...
  loadNewSequence(parsedMidi) {
    ...
    console.log(
      `%cTOTAL TIME: ${formatTime(Math.round(this.duration)).time}`,
  ...
  calculateNoteTimes() {
    ...
    while (eventIndex < events.length) {
      ...
      // note off
      if (status === 0x8) {
        const note = noteTimes[channel].notes.findLast(
          (n) => n.midiNote === event.messageData[0] && n.length === -1
        )
        if (note) {
          const time = elapsedTime - note.start
          note.length =
            time < MIN_NOTE_TIME && channel === DEFAULT_PERCUSSION ? MIN_NOTE_TIME : time
        }
      }
      // note on
      else if (status === 0x9) {
  ...
  ticksToSeconds(ticks) {
    ...
    return (
      this.ticksToSeconds(ticks - timeSinceLastTempo) +
      (timeSinceLastTempo * 60) / (tempo.tempo * this.midiData.timeDivision)
    )
  }
  ...
  _playTo(time, ticks = undefined) {
      ...
      switch (info.status) {
        ...
        case messageTypes.controllerChange:
          ...
          if (
            ...
            if (this.MIDIout) {
              this.MIDIout.send([
                messageTypes.controllerChange | info.channel,
                controllerNumber,
                event.messageData[1]
              ])
            } else {
            ...
            const channel =
              info.channel + (this.midiPortChannelOffsets[this.midiPorts[trackIndex]] || 0)
    ...
    if (this.MIDIout) {
      ...
      for (let channelNumber = 0; channelNumber < 16; channelNumber++) {
        ...
        this.MIDIout.send([
          messageTypes.pitchBend | channelNumber,
  ...
  _processEvent(event, trackIndex) {
    ...
    switch (statusByteData.status) {
        ...
        if (velocity > 0) {
          if (
            (this.synth.highPerformanceMode &&
              this.noteOnsPerS > MAX_NOTEONS_PER_S &&
              velocity < 40) ||
            this.noteOnsPerS > MAX_NOTEONS_PER_S * 2
          ) {
            return
          }
          this.noteOnsPerS++
          ...
          this.playingNotes.splice(
            this.playingNotes.findIndex(
              (n) => n.midiNote === event.messageData[0] && n.channel === statusByteData.channel
            ),
            1
          )
        }
      ...
      default:
        console.log(
          `%cUnrecognized Event: %c${event.messageStatusByte}%c status byte: %c${Object.keys(messageTypes).find((k) => messageTypes[k] === statusByteData.status)}`,
          consoleColors.warn,
