...
export class GeneratorTranslator {
  ...
  constructor(sampleAndGenerators, midiNote, velocity) {
    ...
    this.attenuation =
      (this.sumGeneratorValue(
        ...
        10) *
      EMU_ATTENUATION_CORRECTION

    // delayVolEnv
    this.delayTime = this.sumGeneratorValue(generatorTypes.delayVolEnv, -12000, -12000, 8000)

    // attackVolEnv
    this.attackTime = this.sumGeneratorValue(generatorTypes.attackVolEnv, -12000, -12000, 8000)

    // holdVolEnv
    this.holdTime = this.sumGeneratorValue(generatorTypes.holdVolEnv, -12000, -12000, 5000)

    // decayVolEnv
    this.decayTime = this.sumGeneratorValue(generatorTypes.decayVolEnv, -12000, -12000, 8000)

    // sustainVolEnv (dB)
    this.sustainLowerAmount = this.sumGeneratorValue(generatorTypes.sustainVolEnv, 0, 0, 1440) / 10

    // releaseVolEnv (timecents) defaults to 5s
    this.releaseTime = this.sumGeneratorValue(
    ...
    // offsets
    this.startOffset =
      this.getGeneratorValue(generatorTypes.startAddrsOffset, 0) +
      32768 * this.getGeneratorValue(generatorTypes.startAddrsCoarseOffset, 0)
    this.endOffset =
      this.getGeneratorValue(generatorTypes.endAddrOffset, 0) +
      32768 * this.getGeneratorValue(generatorTypes.endAddrsCoarseOffset, 0)
    this.startLoopOffset =
      this.getGeneratorValue(generatorTypes.startloopAddrsOffset, 0) +
      32768 * this.getGeneratorValue(generatorTypes.startloopAddrsCoarseOffset, 0)
    this.endLoopOffset =
      this.getGeneratorValue(generatorTypes.endloopAddrsOffset, 0) +
      32768 * this.getGeneratorValue(generatorTypes.endloopAddrsCoarseOffset, 0)
    ...
    // modEnvToFilterFc
    this.modFilterInfluence = this.sumGeneratorValue(
  ...
  getVolumeEnvelope() {
    ...
    const holdTime = this.timecentsToSeconds(
      this.holdTime + (60 - this.midiNote) * this.keyToVolHold
    )
    const decayTime = this.timecentsToSeconds(
      this.decayTime + (60 - this.midiNote) * this.keyToVolDecay
    )
  ...
  getFilterEnvelope() {
    ...
    const peakHz = this.limitValue(
      this.absCentsToHz(this.filterCutoff + this.modFilterInfluence),
      FREQ_MIN,
      FREQ_MAX
    )
    ...
    const holdTime = this.timecentsToSeconds(
      this.modHold + (60 - this.midiNote) * this.keyToModHold
    )
    const decayTime = this.timecentsToSeconds(
      this.modDecay + (60 - this.midiNote) * this.keyToModDecay
    )
    const sustainHz = this.limitValue(
      this.absCentsToHz(this.filterCutoff + this.modFilterInfluence * sustainGain) -
        attackHz +
        attackHz,
      FREQ_MIN,
      FREQ_MAX
  ...
  getPlaybackRate() {
    const semitones =
      (this.midiNote - this.getRootKey()) * this.getScaleTuneInfluence() + // tune by key
  ...
  _getSustainLevel() {
    ...
    let sustain =
      this.decibelsToGain(this.attenuation * -1) * this.decibelsToGain(this.sustainLowerAmount * -1)
    if (sustain <= 0) {
      return 0.001
    }
    return sustain
