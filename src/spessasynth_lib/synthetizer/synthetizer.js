...
export class Synthetizer {
  ...
  constructor(targetNode, soundFont) {
    ...
    if (window.isSecureContext === false) {
      ...
      console.log(
        '%cDetected insecure context. Worklet system is unavailable, switching to legacy instead.',
        consoleColors.warn
      )
    }
  ...
  noteOn(channel, midiNote, velocity, enableDebugging = false) {
    ...
    if (
      (this.highPerformanceMode && this.voicesAmount > 200 && velocity < 40) ||
    ...
    if (newVoicesAmount > this.voiceCap) {
      ...
      const channel = this.midiChannels.reduce((prev, current) => {
        return prev && prev.voicesAmount > current.voicesAmount && !prev.percussionChannel
          ? prev
          : current
      })
  ...
  resetControllers() {
    ...
    for (const ch of this.midiChannels) {
      ...
      this.eventHandler.callEvent('controllerchange', {
        channel: chNr,
        controllerNumber: midiControllers.expressionController,
        controllerValue: 127
      })
      this.eventHandler.callEvent('controllerchange', {
        channel: chNr,
        controllerNumber: midiControllers.modulationWheel,
        controllerValue: 0
      })
      this.eventHandler.callEvent('controllerchange', {
        channel: chNr,
        controllerNumber: midiControllers.effects3Depth,
        controllerValue: 0
      })
      this.eventHandler.callEvent('controllerchange', {
        channel: chNr,
        controllerNumber: midiControllers.effects1Depth,
        controllerValue: 0
      })
    ...
    switch (type) {
      ...
      case 0x41:
        ...
        if (messageData[2] === 0x42 && messageData[3] === 0x12) {
          ...
          if (messageData[7] === 0x00 && messageData[6] === 0x7f) {
            ...
            console.log(
              `%cChannel %c${channel}%c ${
                this.midiChannels[channel].percussionChannel
            ...
            // roland master pan
            console.log(
              `%cRoland Master Pan set to: %c${messageData[7]}%c with: %c${arrayToHexString(messageData)}`,
              consoleColors.info,
              consoleColors.value,
              consoleColors.info,
              consoleColors.value
          ...
          console.log(
            `%cRoland Master Volume control set to: %c${messageData[7]}%c via: %c${arrayToHexString(messageData)}`,
            consoleColors.info,
            consoleColors.value,
            consoleColors.info,
            consoleColors.value
