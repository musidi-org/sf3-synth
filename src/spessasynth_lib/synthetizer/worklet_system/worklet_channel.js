...
export class WorkletChannel {
  ...
  getWorkletVoices(midiNote, velocity) {
    ...
    if (cached) {
       ...
       * @returns {WorkletVoice}
       */
      workletVoices = this.preset
        .getSamplesAndGenerators(midiNote, velocity)
        .map((sampleAndGenerators) => {
          ...
          if (!this.dumpedSamples.has(sampleAndGenerators.sampleID)) {
            ...
            this.post({
              ...
              messageData: {
                ...
                sampleData: sampleAndGenerators.sample.getAudioData()
              }
            })
          }
          ...
          const workletSample = {
            sampleID: sampleAndGenerators.sampleID,
            playbackStep:
              (sampleAndGenerators.sample.sampleRate / this.ctx.sampleRate) *
              Math.pow(2, sampleAndGenerators.sample.samplePitchCorrection / 1200), // cent tuning
            cursor:
              generators[generatorTypes.startAddrsOffset] +
              generators[generatorTypes.startAddrsCoarseOffset] * 32768,
            rootKey: rootKey,
            loopStart:
              sampleAndGenerators.sample.sampleLoopStartIndex / 2 +
              (generators[generatorTypes.startloopAddrsOffset] +
                generators[generatorTypes.startloopAddrsCoarseOffset] * 32768),
            loopEnd:
              sampleAndGenerators.sample.sampleLoopEndIndex / 2 +
              (generators[generatorTypes.endloopAddrsOffset] +
                generators[generatorTypes.endloopAddrsCoarseOffset] * 32768),
            end:
              sampleAndGenerators.sample.sampleLength / 2 +
              1 +
              (generators[generatorTypes.endAddrOffset] +
                generators[generatorTypes.endAddrsCoarseOffset] * 32768),
  ...
  setPitchBendRange(semitones) {
    ...
    console.log(
      `%cChannel ${this.channelNumber} bend range. Semitones: %c${semitones}`,
      consoleColors.info,
      consoleColors.value
    )
    this.post({
          ...
          case 1:
            switch (this.NRPFine) {
              ...
              case 8:
                ...
                console.log(
                  `%cVibrato rate for channel %c${this.channelNumber}%c is now set to %c${this._vibrato.rate}%cHz.`,
                  consoleColors.info,
              ...
              case 9:
                ...
                console.log(
                  `%cVibrato depth for %c${this.channelNumber}%c is now set to %c${this._vibrato.depth} %ccents range of detune.`,
                  consoleColors.info,
                  consoleColors.recognized,
                  consoleColors.info,
                  consoleColors.value,
              ...
              case 10:
                ...
                console.log(
                  `%cVibrato delay for %c${this.channelNumber}%c is now set to %c${this._vibrato.delay} %cseconds.`,
                  consoleColors.info,
                  consoleColors.recognized,
                  consoleColors.info,
                  consoleColors.value,
  ...
  setChannelTuning(semitones) {
    ...
    console.log(
      `%cChannel ${this.channelNumber} tuning. Semitones: %c${this.channelTuningSemitones}`,
      consoleColors.info,
      consoleColors.value
    )
    this.post({
      messageType: workletMessageType.ccChange,
      messageData: [
        NON_CC_INDEX_OFFSET + modulatorSources.channelTuning,
        this.channelTuningSemitones * 100
      ]
    })
  }
  ...
  transposeChannel(semitones, force = false) {
    ...
    this.post({
      messageType: workletMessageType.ccChange,
      messageData: [
        NON_CC_INDEX_OFFSET + modulatorSources.channelTranspose,
        this.channelTranspose * 100
      ]
    })
  }
