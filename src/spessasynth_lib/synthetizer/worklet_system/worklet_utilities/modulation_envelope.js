...
export function getModEnvValue(voice, currentTime) {
  ...
  let decay = timecentsToSeconds(
    voice.modulatedGenerators[generatorTypes.decayModEnv] +
      (60 - voice.midiNote) * voice.modulatedGenerators[generatorTypes.keyNumToModEnvDecay]
  )
  let hold = timecentsToSeconds(
    voice.modulatedGenerators[generatorTypes.holdModEnv] +
      (60 - voice.midiNote) * voice.modulatedGenerators[generatorTypes.keyNumToModEnvHold]
  )
  ...
  let delayEnd =
    timecentsToSeconds(voice.modulatedGenerators[generatorTypes.delayModEnv]) + voice.startTime
