...
export function applyLowpassFilter(voice, outputBuffer, cutoffCents) {
  ...
  if (
    ...
    voice.filter.reasonanceGain = decibelAttenuationToGain(
      -1 * (voice.filter.reasonanceCb / 10 - 3.01)
    ) // -1 because it's attenuation that we're inverting
