...
export class SynthetizerUI {
    ...
    document.addEventListener('keydown', (e) => {
      switch (e.key.toLowerCase()) {
        case 's':
          ...
          controller
            .getElementsByClassName('controls_wrapper')[0]
            .classList.toggle('controls_wrapper_show')
  ...
  createMainSynthController() {
    ...
    this.panController = new Meter(
      ...
      (v) => {
        ...
        this.synth.systemExclusive(
          new ShiftableByteArray([0x41, 0x10, 0x42, 0x12, 0x40, 0x00, 0x06, ((v + 1) / 2) * 127])
        )
      }
    )
    ...
    highPerfToggle.title =
      'Toggles the High Performance Mode, simplifying the look and killing the notes faster'
  ...
  createChannelController(channel, channelNumber) {
    ...
    const chorus = new Meter(
      ...
      0,
      127,
      `The current level of chorus effect applied to channel ${channelNumber + 1}`,
      true,
      (val) => {
    ...
    const reverb = new Meter(
      ...
      0,
      127,
      `The current level of reverb effect applied to channel ${channelNumber + 1}`,
      true,
      (val) => {
    ...
    const transpose = new Meter(
      ...
      -36,
      36,
      `The pitch tuning applied to channel ${channelNumber + 1}`,
      true,
      (val) => {
        val = Math.round(val)
    ...
    drumsToggle.onclick = () => {
      ...
      const sysexChannelNumber = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 10, 11, 12, 13, 14, 15][
        channelNumber
      ]
      this.synth.systemExclusive(
        new ShiftableByteArray([
          // roland gs drum sysex
          0x41, // roland
  ...
  setEventListeners() {
    ...
    this.synth.eventHandler.addEvent('drumchange', 'synthui-drum-change', (e) => {
      ...
      this.controllers[e.channel].drumsToggle.innerHTML = e.isDrumChannel
        ? getDrumsSvg(32)
        : getNoteSvg(32)
      this.controllers[e.channel].preset.reload(
  ...
  getInstrumentList() {
     ...
     */
    this.instrumentList = this.synth.soundFont.presets
      .filter((p) => p.bank !== 128)
      .sort((a, b) => {
     ...
     * @type {{name: string, program: number, bank: number}[]}
     */
    this.percussionList = this.synth.soundFont.presets
      .filter((p) => p.bank === 128)
  ...
  reloadSelectors() {
    this.getInstrumentList()
    this.controllers.forEach((controller, i) => {
      controller.preset.reload(
        this.synth.midiChannels[i].percussionChannel ? this.percussionList : this.instrumentList
      )
    })
  }
}
