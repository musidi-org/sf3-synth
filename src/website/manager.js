    ...
    if (context.audioWorklet) {
      try {
        await context.audioWorklet.addModule(
          '/spessasynth_lib/synthetizer/worklet_system/channel_processor.js'
        )
      } catch (e) {
        await context.audioWorklet.addModule(
          '/SpessaSynth/src/spessasynth_lib/synthetizer/worklet_system/channel_processor.js'
        )
      }
    }
    // set up soundfont
        ...
        case 'v':
          ...
          if (this.seq) {
            video.currentTime = parseFloat(
              window.prompt('Video offset to sync to midi, in seconds.', '0')
            )
            video.play()
